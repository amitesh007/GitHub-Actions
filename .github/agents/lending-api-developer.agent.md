...
11. **Step 11:** Calculate token usage and write `usage-agent.md` (ALWAYS — executes after all other steps)
...
- Step 11 executes always as the final action — records token usage, rates, cost, and wall-clock time to `FLIQ-liqjava/usage-agent.md` (append-only log)
...
## Step 11: Calculate Token Usage and Write usage-agent.md

**Note:** This step ALWAYS executes as the very last action, after all other steps (including Step 10) complete — regardless of whether the workflow succeeded, had conflicts, or encountered errors.

**Purpose:** Provide full cost and performance transparency for every agent run by recording exact token consumption, per-token rates, total cost, and wall-clock execution time in a persistent, append-only log file at `FLIQ-liqjava/usage-agent.md`.

### 11.1 Collect Runtime Metrics

Throughout the workflow execution, the agent MUST track the following cumulative counters. Increment each counter as calls are made:

| Metric | How to Collect |
|--------|----------------|
| **Start Timestamp** | Record ISO-8601 datetime when Step 1 begins (before PowerShell script execution) |
| **End Timestamp** | Record ISO-8601 datetime immediately before writing usage-agent.md |
| **Input Tokens** | Sum of all `usage.input_tokens` values across every model API call in this session |
| **Output Tokens** | Sum of all `usage.output_tokens` values across every model API call in this session |
| **Cache Read Tokens** | Sum of all `usage.cache_read_input_tokens` values across every model API call |
| **Cache Write Tokens** | Sum of all `usage.cache_creation_input_tokens` values across every model API call |
| **Model Name** | Extract from agent frontmatter `model:` field (e.g., `claude-sonnet-4.6`) |

**If runtime token data is not directly accessible**, estimate from session context:
- Use the total character count of all inputs × 0.25 as an approximation for input tokens
- Use the total character count of all outputs × 0.25 as an approximation for output tokens
- Mark estimated values with `*` suffix in the table (e.g., `12500*`)
- Add a footnote: `* Estimated (runtime token API unavailable)`

### 11.2 Apply Model Pricing Rates

Use the following pricing table for `claude-sonnet-4.6` (rates per 1,000,000 tokens):
...
**If the model in the frontmatter differs from `claude-sonnet-4.6`**, look up the correct rates from `.github/hooks/lending-model-health-check/scripts/check-model-versions.mjs` or use the Anthropic pricing page. Never apply `claude-sonnet-4.6` rates to a different model.

### 11.3 Calculate Derived Values

Compute the following before writing the file:

```
Input Cost       = (Input Tokens / 1,000,000) × Input Rate
Output Cost      = (Output Tokens / 1,000,000) × Output Rate
Cache Read Cost  = (Cache Read Tokens / 1,000,000) × Cache Read Rate
Cache Write Cost = (Cache Write Tokens / 1,000,000) × Cache Write Rate
Total Tokens     = Input Tokens + Output Tokens + Cache Read Tokens + Cache Write Tokens
Total Price      = Input Cost + Output Cost + Cache Read Cost + Cache Write Cost
Total Time       = End Timestamp − Start Timestamp (formatted as HH:MM:SS or Xm Ys)
```

### 11.4 Write usage-agent.md

**File location:** `FLIQ-liqjava/usage-agent.md`

**Behavior:**
- If the file **does not exist**: create it with a header row followed by the new entry row.
- If the file **already exists**: append a new entry row after the last existing row (do NOT overwrite previous runs — this is an append-only log).

**File format (Markdown table):**

```markdown
# Agent Usage Log — Lending API Developer

> Append-only log. Each row represents one complete agent execution run.

| Run # | Date & Time (Start) | Entity | API Types | Model Name Used | Input Token | Input Rate ($/1M) | Input Cost ($) | Output Token | Output Rate ($/1M) | Output Cost ($) | Cache Read Token | Cache Read Rate ($/1M) | Cache Read Cost ($) | Cache Write Token | Cache Write Rate ($/1M) | Cache Write Cost ($) | Total Token | Total Price ($) | Total Time |
|-------|---------------------|--------|-----------|----------------|------------|-------------------|---------------|-------------|-------------------|----------------|-----------------|----------------------|---------------------|------------------|------------------------|---------------------|------------|----------------|------------|
| 1 | {StartTimestamp} | {EntityName} | {Create/Update/Query/Delete} | {ModelName} | {InputTokens} | 3.00 | {InputCost} | {OutputTokens} | 15.00 | {OutputCost} | {CacheReadTokens} | 0.30 | {CacheReadCost} | {CacheWriteTokens} | 3.75 | {CacheWriteCost} | {TotalTokens} | {TotalPrice} | {TotalTime} |
```

**Rules for appending:**
1. Read the existing file to determine the last `Run #` value; increment by 1 for the new row.
2. If no prior rows exist, start at `Run # = 1`.
3. **Individual costs** (each formatted to 6 decimal places):
   - `Input Cost ($)`       = `InputTokens / 1,000,000 × 3.00`
   - `Output Cost ($)`      = `OutputTokens / 1,000,000 × 15.00`
   - `Cache Read Cost ($)`  = `CacheReadTokens / 1,000,000 × 0.30`
   - `Cache Write Cost ($)` = `CacheWriteTokens / 1,000,000 × 3.75`
4. **Total Token** = `InputTokens + OutputTokens + CacheReadTokens + CacheWriteTokens` — plain integer, no comma separators.
5. **Total Price ($)** = sum of all four individual costs — format to 6 decimal places (e.g., `0.342963`).
6. Format token counts as plain integers with no comma separators (e.g., `15230`).
7. Format Total Time as `Xm Ys` (e.g., `4m 32s`) or `HH:MM:SS` for runs over 1 hour.
8. If any metric is unavailable, write `N/A` in that cell.

**Example completed row:**

```
| 1 | 2026-07-24 14:32:10 | LoanPrincipalPayment | Create, Update, Query, Delete | claude-sonnet-4.6 | 45230 | 3.00 | 0.135690 | 12840 | 15.00 | 0.192600 | 8910 | 0.30 | 0.002673 | 3200 | 3.75 | 0.012000 | 70180 | 0.342963 | 8m 47s |
```

### 11.5 Output Confirmation

After writing the file, output the following summary block:

```
## Step 11: Token Usage Recorded

**File:** FLIQ-liqjava/usage-agent.md
**Run #:** {RunNumber}
**Entity:** {EntityName}
**Model Name Used:** {ModelName}
...
| Output Rate ($/1M)        | 15.00                  |
| Output Cost ($)           | {OutputCost}           |
| Cache Read Token          | {CacheReadTokens}      |
| Cache Read Rate ($/1M)    | 0.30                   |
| Cache Read Cost ($)       | {CacheReadCost}        |
| Cache Write Token         | {CacheWriteTokens}     |
| Cache Write Rate ($/1M)   | 3.75                   |
| Cache Write Cost ($)      | {CacheWriteCost}       |
| Total Token               | {TotalTokens}          |
| Total Price ($)           | {TotalPrice}           |
| Total Time to Execute     | {TotalTime}            |

**Usage log appended to:** `FLIQ-liqjava/usage-agent.md`
```

### 11.6 Update Emit Manifest

Append `FLIQ-liqjava/usage-agent.md` to the `generated-files` list in the Step 10 manifest:
...
**Post-condition Validation:**
- [ ] `usage-agent.md` exists at `FLIQ-liqjava/usage-agent.md`
- [ ] New row appended (not overwritten) if file existed
- [ ] All token counts are non-negative integers
- [ ] All cost fields are non-negative decimals formatted to 6 decimal places
- [ ] Run number incremented correctly from last entry
- [ ] Total Time reflects actual wall-clock duration of this agent execution

**Output:** `FLIQ-liqjava/usage-agent.md` updated with new usage entry row and Step 11 cost breakdown summary printed to console.
